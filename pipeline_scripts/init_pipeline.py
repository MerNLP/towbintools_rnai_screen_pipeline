...
from pipeline_scripts.utils import filter_filemap_image_rows
...
def main(global_config, temp_dir_basename, temp_dir, subdir=None):
    ...
    if not os.path.exists(
        ...
        if experiment_filemap.is_empty():
            image_paths = list_image_paths(raw_subdir)
    ...
    n_before = experiment_filemap.height
    experiment_filemap = filter_filemap_image_rows(experiment_filemap, raw_dir_name)
    if experiment_filemap.height != n_before:
        print(
            f"### Removed {n_before - experiment_filemap.height} non-image row(s) "
            "from filemap ###"
        )
        write_filemap(experiment_filemap, filemap_path)
