...
def is_image_path(path, pattern: str = DEFAULT_IMAGE_PATTERN) -> bool:
    """Return True if ``path`` looks like an image file for the given glob pattern."""
    if path in (None, ""):
        return False
    return fnmatch(Path(str(path)).name, pattern)


def list_image_paths(directory: str, pattern: str = DEFAULT_IMAGE_PATTERN) -> list[str]:
    """Return sorted image file paths under ``directory`` matching ``pattern``."""
    root = Path(directory)
    if not root.is_dir():
        return []
...
def map_paths_by_basename(
    directory_paths: list[str], target_basenames: list[str]
) -> list[str]:
    """Map each basename to a path from ``directory_paths``, or ``""`` if missing."""
    by_name = {os.path.basename(path): path for path in directory_paths}
    return [by_name.get(name, "") for name in target_basenames]


def filter_filemap_image_rows(
    experiment_filemap: pl.DataFrame,
    raw_col: str,
    pattern: str = DEFAULT_IMAGE_PATTERN,
) -> pl.DataFrame:
    """Drop filemap rows whose raw path is not an image file."""
    if raw_col not in experiment_filemap.columns:
        return experiment_filemap
    return experiment_filemap.filter(
        pl.col(raw_col).map_elements(
            lambda path: is_image_path(path, pattern),
...
def get_experiment_subdirs(config):
    ...
    only = config.get("experiment_subdirs")
    if only:
        wanted = {str(s) for s in only}
        subdirs = [s for s in subdirs if s in wanted]
        missing = wanted - set(subdirs)
        if missing:
            raise ValueError(
                f"experiment_subdirs not found under {raw_subdir}: {sorted(missing)}"
            )
    return subdirs


def _analysis_dir_is_writable(path: str) -> bool:
    """Return True if ``path`` exists (or can be created) and is writable."""
    try:
        os.makedirs(path, exist_ok=True)
        probe = os.path.join(path, ".pipeline_write_test")
...
def resolve_analysis_subdir(config) -> tuple[str, str]:
    """
    Pick where segmentation / analysis outputs are written.

    1. ``analysis_output_dir`` when set and writable (optional override)
    2. ``experiment_dir`` / ``analysis_dir_name`` (default ``analysis``)
    """
    experiment_dir = config["experiment_dir"]
    analysis_name = str(config.get("analysis_dir_name", "analysis"))
    primary = config.get("analysis_output_dir")

    if primary not in (None, ""):
        primary_path = os.path.abspath(str(primary))
        if _analysis_dir_is_writable(primary_path):
            print(f"### Analysis outputs -> {primary_path} ###")
            return primary_path, analysis_name
        raise OSError(
            f"analysis_output_dir is not writable: {primary_path}"
        )

    analysis_path = os.path.join(experiment_dir, analysis_name)
    if _analysis_dir_is_writable(analysis_path):
        print(f"### Analysis outputs -> {analysis_path} ###")
        return analysis_path, analysis_name

    raise OSError(
        "No writable analysis directory found. "
        f"Tried analysis_output_dir={primary!r}, "
        f"experiment_dir/analysis_dir_name={analysis_path!r}."
    )
...
def get_and_create_folders(config, subdir=None):
    ...
    analysis_subdir, analysis_label = resolve_analysis_subdir(config)
    config["analysis_dir_name"] = analysis_label
...
def get_input_and_output_files(experiment_filemap, columns, output_dir, rerun=True):
    ...
    all_input_files = [
        row
        for row in all_input_files
        if all(is_image_path(cell) for cell in row if cell not in (None, ""))
    ]
...
def run_command(
    ...
    # Only GPU blocks should use sbatch_partition (e.g. "gpu"). CPU morphology
    # jobs must stay on the default partition like before.
    partition = config.get("sbatch_partition") if requires_gpu else None
    nodelist = config.get("sbatch_nodelist") if requires_gpu else None
    sbatch_kwargs = dict(
        partition=partition,
        nodelist=nodelist,
    ...
    if result.returncode != 0:
        raise RuntimeError(
            f"sbatch failed for {script_name} ({script_path}):\n{result.stderr.strip()}"
        )
...
"""
    if partition not in (None, ""):
        content += f"#SBATCH -p {partition}\n"
    if nodelist not in (None, ""):
        content += f"#SBATCH -w {nodelist}\n"
    if gpus is not None:
